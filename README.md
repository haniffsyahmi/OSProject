...
```bash
@haniffsyahmi ➜ /workspaces/OSProject (main) $ pwd
/workspaces/OSProject
```
2. Run the command **cat /etc/passwd** . ***(1 mark)***
```bash
@haniffsyahmi ➜ /workspaces/OSProject (main) $ cat /etc/passwd
...
```bash
@haniffsyahmi ➜ /workspaces/OSProject (main) $ df
Filesystem     1K-blocks     Used Available Use% Mounted on
overlay         32847680 10380796  20772788  34% /
tmpfs              65536        0     65536   0% /dev
shm                65536        8     65528   1% /dev/shm
/dev/root       30298176 24360932   5920860  81% /vscode
/dev/sdb1       46127956       96  43752284   1% /tmp
/dev/loop3      32847680 10380796  20772788  34% /workspaces
```
4. Run the command **du** . ***(1 mark)***
```bash
@haniffsyahmi ➜ /workspaces/OSProject (main) $ du
1972    ./images
...
```bash
@haniffsyahmi ➜ /workspaces/OSProject (main) $ ls
README.md  images
```
6. Run the command **ls -asl** . ***(1 mark)*** 
```bash
@haniffsyahmi ➜ /workspaces/OSProject (main) $ ls -asl
total 36
 4 drwxrwxrwx+ 4 codespace root  4096 Jun 30 11:47 .
 4 drwxr-xrwx+ 5 codespace root  4096 Jun 30 11:47 ..
 4 drwxrwxrwx+ 9 codespace root  4096 Jun 30 11:49 .git
20 -rw-rw-rw-  1 codespace root 18164 Jun 30 11:47 README.md
 4 drwxrwxrwx+ 2 codespace root  4096 Jun 30 11:47 images
```
7. Run the command **free -h** . ***(1 mark)*** 
```bash
@haniffsyahmi ➜ /workspaces/OSProject (main) $ free -h
              total        used        free      shared  buff/cache   available
Mem:          7.7Gi       1.3Gi       248Mi        65Mi       6.2Gi       6.0Gi
Swap:            0B          0B          0B
```
8. Run the command **cat /proc/cpuinfo** . ***(1 mark)***
```bash
@haniffsyahmi ➜ /workspaces/OSProject (main) $ cat /proc/cpuinfo
processor       : 0
...
flags           : fpu vme de pse tsc msr pae mce cx8 apic sep mtrr pge mca cmov pat pse36 clflush mmx fxsr sse sse2 ht syscall nx mmxext fxsr_opt pdpe1gb rdtscp lm constant_tsc rep_good nopl tsc_reliable nonstop_tsc cpuid extd_apicid aperfmperf pni pclmulqdq ssse3 fma cx16 pcid sse4_1 sse4_2 movbe popcnt aes xsave avx f16c rdrand hypervisor lahf_lm cmp_legacy svm cr8_legacy abm sse4a misalignsse 3dnowprefetch osvw topoext invpcid_single vmmcall fsgsbase bmi1 avx2 smep bmi2 erms invpcid rdseed adx smap clflushopt clwb sha_ni xsaveopt xsavec xgetbv1 xsaves clzero xsaveerptr rdpru arat npt nrip_save tsc_scale vmcb_clean flushbyasid decodeassists pausefilter pfthreshold v_vmsave_vmload umip vaes vpclmulqdq rdpid fsrm
bugs            : sysret_ss_attrs null_seg spectre_v1 spectre_v2 spec_store_bypass srso
top - 11:59:47 up 22 min,  0 users,  load average: 0.09, 0.19, 0.32
Tasks:  20 total,   1 running,  19 sleeping,   0 stopped,   0 zombie
%Cpu(s):  2.7 us,  3.5 sy,  0.0 ni, 93.6 id,  0.2 wa,  0.0 hi,  0.0 si,  0.0 st
MiB Mem :   7929.6 total,    252.8 free,   1353.5 used,   6323.2 buff/cache
MiB Swap:      0.0 total,      0.0 free,      0.0 used.   6194.4 avail Mem 

    PID USER      PR  NI    VIRT    RES    SHR S  %CPU  %MEM     TIME+ COMMAND                                     
   2469 codespa+  20   0   21.5g 329772  48000 S   1.3   4.1   0:13.47 node                                        
      1 codespa+  20   0    1136    640    640 S   0.0   0.0   0:00.03 docker-init                                 
```
10. Run the command **uname -a**. ***(1 mark)*** 
```bash
@haniffsyahmi ➜ /workspaces/OSProject (main) $ uname -a
Linux codespaces-77357e 6.5.0-1022-azure #23~22.04.1-Ubuntu SMP Thu May  9 17:59:24 UTC 2024 x86_64 x86_64 x86_64 GNU/Linux
```
...
```bash
Ubuntu 22.04.1 and a 6.5.0-1022-azure Linux kernel
```
14. What is the difference between **ls** vs **ls -asl**. ***(1 mark)*** 
```bash
`ls` gives a basic directory listing, while `ls -asl` shows all files with detailed information like size and permissions.
```
...
```bash
Processor 0: cpu MHz     : 3035.505 (approximately 3.04 GHz)
Processor 1: cpu MHz     : 2599.484 (approximately 2.60 GHz)
```
